...
export async function notifyEarner (userId, msats) {
  const title = `you stacked ${numWithUnits(msatsToSats(msats))} in rewards`

  try {
    await sendUserNotification(userId, { title, setting: 'noteEarning' })
  } catch (err) {
